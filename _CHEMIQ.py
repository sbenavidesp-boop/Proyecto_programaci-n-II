## CHEMIQ – Calculadora de masa molecular con instalación automática de librerías,
...
def resolver_ruta_logo():
    """Busca el logo en la carpeta actual o Descargas."""
    bases = [
        Path.cwd(),
        Path.home() / "_CHEMIQ",
    ]
...
def cargar_logo(max_width=420, max_height=180):
    """Carga y escala el logo, con o sin Pillow."""
...
def construir_tabla_periodica(base_local: dict):
    """Intenta construir la tabla periódica con librerías externas."""
    try:
...
def leer_elemento(s, i):
    ...
    if elem not in TABLA:
        raise KeyError(
            f"Elemento '{elem}' no encontrado (fuente: {FUENTE_TABLA}).\n"
            f"Verifica mayúsculas/minúsculas ('Na', 'Fe', 'Cl').\n"
            f"Instala periodictable o mendeleev si falta la tabla completa."
        )
    i = m.end()
    mult, i = leer_num(s, i)
...
def abrir_historial(ventana):
    ...
    tree.pack(padx=12, pady=(12, 6), fill="both", expand=True)

    for item in cargar_historial():
        tree.insert("", "end", values=(item["formula"], f'{item["masa"]:.4f}', item["timestamp"].replace("T", " ")))
    ...
    tk.Button(cont, text="🗑️ Limpiar historial", command=limpiar, bg="#c62828", fg="white",
              font=("Segoe UI", 10), padx=10, pady=5).grid(row=0, column=0, padx=6)
    tk.Button(cont, text="Cerrar", command=win.destroy, bg="#444", fg="white",
              font=("Segoe UI", 10), padx=10, pady=5).grid(row=0, column=1, padx=6)
...
def calcular(ventana, entrada_formula, etiqueta_resultado):
    ...
    try:
        ...
        etiqueta_resultado.config(text=f"Masa molecular de {formula}: {resultado:.4f} g/mol")
...
def abrir_flujo_datos():
    ...
    for texto, x, y, color in nodos:
        c = canvas.create_oval(x-100, y-30, x+100, y+30, fill=color, width=0, state="hidden")
        t = canvas.create_text(x, y, text=texto, fill="white", font=("Segoe UI", 12, "bold"), state="hidden")
        circulos.append(c)
        textos.append(t)

    def animar(i=0):
        if i >= len(nodos): return
        ...
        if i < len(nodos)-1:
            ...
            flecha = canvas.create_line(x1, y1, x2, y2, arrow=tk.LAST, width=3, fill="#b3e5fc", state="hidden")
            def mostrar():
                canvas.itemconfigure(flecha, state="normal")
                canvas.after(500, lambda: animar(i+1))
            canvas.after(400, mostrar)
        else:
            canvas.after(500, lambda: canvas.create_text(320, 30,
                text="💡 Flujo completo de datos", fill="white",
                font=("Segoe UI", 14, "bold")))
    animar()

    btn_close = tk.Button(
        win, text="Cerrar", command=win.destroy,
        bg="#6a38d9", fg="white", font=("Segoe UI", 10, "bold"),
        padx=10, pady=4, relief="flat", cursor="hand2"
    )
    btn_close.place(relx=1.0, rely=0.0, x=-12, y=10, anchor="ne")
...
def construir_contenido(frame, ventana):
    ...
    entrada_formula = tk.Entry(frame, width=32, font=("Segoe UI", 12), relief="solid", bd=1)
    entrada_formula.pack(pady=(0, 10))
    entrada_formula.focus_set()

    cont = tk.Frame(frame, bg="#ffffff")
    cont.pack(pady=6)
    tk.Button(cont, text="Calcular", command=lambda: calcular(ventana, entrada_formula, etiqueta_resultado),
              font=("Segoe UI", 11), bg="#0b3d91", fg="white", padx=12, pady=6).grid(row=0, column=0, padx=6)
    tk.Button(cont, text="Ayuda", command=mostrar_ayuda,
              font=("Segoe UI", 11), bg="#026a9f", fg="white", padx=12, pady=6).grid(row=0, column=1, padx=6)
    tk.Button(cont, text="Historial", command=lambda: abrir_historial(ventana),
              font=("Segoe UI", 11), bg="#5a2ea6", fg="white", padx=12, pady=6).grid(row=0, column=2, padx=6)
    tk.Button(cont, text="Flujo de Datos", command=abrir_flujo_datos,
              font=("Segoe UI", 11), bg="#00838f", fg="white", padx=12, pady=6).grid(row=0, column=3, padx=6)

    global etiqueta_resultado
    etiqueta_resultado = tk.Label(frame, text="", bg="#ffffff", font=("Segoe UI", 12), fg="#111")
    etiqueta_resultado.pack(pady=(14, 6))
...
def crear_ventana():
    ...
    frame = tk.Frame(canvas, bg="#ffffff", highlightbackground="#ccc", highlightthickness=1)
    ...
    def on_resize(event):
        ...
        if ventana._logo_img:
            ...
            canvas.create_text(w // 2, 90, text="CHEMIQ", fill="white",
                               font=("Segoe UI", 24, "bold"), tags="logo")
